...
from transformers import RobertaTokenizer, XLNetTokenizer, DistilBertTokenizer,BertTokenizer,XLMRobertaTokenizer
...
class ParseTree():
    ...
    def traverse_and_store(self,
        ...
        words = " ".join(words)
        if tree.height() > self.TREE_HEIGHT and ngram_info < self.NGRAM_LIMIT and ngram_info > 1:
    ...
    def get_parse_tree_for_raw_sent(self, raw_sent):
        # if(~self.tokenizer.tokenize(raw_sent)):
        #     return("Error")
        ...
        try:
            parsed_tree = self.parser.parse(sentence=combined_text[:-1])
            parsed_tree = ParentedTree.convert(parsed_tree)
            parsed_tree = self.add_indices_to_terminals(parsed_tree)
            ...
            nt_idx_matrix = []
            for x in parsed_tree_as_list:
                nt_idx_matrix.append(x['onehot'])
